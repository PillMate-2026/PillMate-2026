...
window.openEditExpirationModal = function () {
  ...
  const currentExpirationDate = document
    .getElementById("modalExpirationDate")
    .textContent.trim()
    .slice(0, 10);
...
window.submitEditExpiration = async function () {
  const newExpirationDate = document.getElementById(
  ...
  try {
    const response = await fetch(
      `/api/medicine-detail/${currentMedicineId}/expiration`,
      {
        method: "PATCH",
        headers: {
...
window.openMedicineDetailModal = async function (id) {
  ...
  try {
    ...
    document.getElementById("modalUseMethod").textContent = cleanText(
      medicine.use_method,
    );
    ...
    document.getElementById("modalSideEffect").textContent = cleanText(
